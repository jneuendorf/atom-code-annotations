...
export default class AnnotationManager {
    ...
    static deleteAnnotation(editor, lineNumber, index) {
        ...
        this.showContainer({annotations: annotationsDb.getAnnotations(lineNumber)})
        this._reinitializeEditor(editor)
    }
    ...
    static _initEditor(editor) {
        ...
        const gutter = editor.addGutter({
            name: GUTTER_NAME,
            priority: atom.config.get('code-annotations.gutter.priority'),
            visible: true,
        })
        ...
        for (const [lineNumberStr, annotations] of Object.entries(annotationsByLine)) {
            ...
            gutterIcon.classList.add(
                ...
                'icon',
                `icon-${atom.config.get('code-annotations.gutter.icon')}`,
            )
            logger.info('using icon', atom.config.get('code-annotations.gutter.icon'))
