...
export default class AnnotationsDb {
    ...
    constructor(projectDir) {
        if (!annotationManager) {
            throw new Error(
                '`AnnotationsDb.setAnnotationManager` must be called '
        ...
        this.file.onDidChange(this._handleFileChange)
        this.file.onDidRename(this._handleFileDelete)
        this.file.onDidDelete(this._handleFileDelete)

        AnnotationsDb._instances.push(this)
    }

    static setAnnotationManager(manager) {
    ...
    persist() {
        annotationManager.updateTreeViewIcons()
        return new Promise((resolve, reject) => {
    ...
    _handleFileChange = async () => {
        await this.readAnnotations()
        await annotationManager.reloadEditors(Object.keys(this.annotations))
    }
