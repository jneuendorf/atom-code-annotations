...
export default class AnnotationManager {
    ...
    static activate(state) {
        ...
        // Static methods can't be bound like instance methods.
        this.deleteAnnotation = this.deleteAnnotation.bind(this)
        this.editAnnotation = this.editAnnotation.bind(this)

        this.treeViewConsumptionPromise = new Promise((resolve, reject) => {
            this.resolveTreeViewConsumptionPromise = resolve
        })
        ...
        window.setTimeout(
            async () => {
                this.subscriptions.add(
                    ...
                    atom.workspace.observeActiveTextEditor(editor => {
                        if (editor instanceof TextEditor) {
                            ...
                            const isDbFile = (
                                editorPath
                                && path.basename(editorPath) === ANNOTATIONS_DB_FILENAME
                            )
                            if (!isDbFile) {
                ...
                AnnotationsDb.setAnnotationManager(this)
                await this._loadAnnotationsDbs(directories)
                await this.loadEditor(atom.workspace.getActiveTextEditor())

                // Update icons "in the background".
                this.treeViewConsumptionPromise.then(() => this.updateTreeViewIcons())
    ...
    static consumeTreeView(treeView) {
        if (!Config.get('treeView.highlightAnnotated')) {
            return
        }

        this.treeView = treeView
        this.resolveTreeViewConsumptionPromise()
        this.mutationObserver = new MutationObserver(mutationsList => {
            this.updateTreeViewIcons()
        })

        // Wait some time for the DOM and make sure package activation doesn't
        // fail because of some hacky UI stuff.
        setTimeout(() => {
            try {
                ...
                if (treeViewElement) {
                    ...
                    const message = (
                        'Cannot observe tree view DOM element because it was '
                        + 'not found. If you can reproduce the problem please '
                        + 'file an issue at '
                        + 'https://github.com/jneuendorf/atom-code-annotations/issues.'
                    )
                    atom.notifications.addWarning(message, {dismissable: true})
                }
            }
            catch (error) {
                console.error('Error while tree view observation:', error)
            }
        }, 500)
    }
    ...
    static updateTreeViewIcons() {
        if (!Config.get('treeView.highlightAnnotated')) {
            return
        }

        const fileIconsPackage = atom.packages.getActivePackage('file-icons')
        const annotatedFilePaths = new Set(
            AnnotationsDb.getAll()
            .map(db =>
                Object.keys(db.annotations)
                .map(filePath => path.join(db.getProjectPath(), filePath))
            )
            .flatten()
        )
        logger.log('annotatedFilePaths', annotatedFilePaths)

        const className = `annotated${fileIconsPackage ? '--file-icons' : ''}`
        // Find all currently highlighted tree-view entries and remove the
        // highlighting from all that are no longer annotated.
        document
        .querySelectorAll(`.tree-view .file.entry.${className} .name.icon`)
        .forEach(icon => {
            if (!annotatedFilePaths.has(icon.dataset.path)) {
                icon.closest('.file.entry').classList.remove(className)
            }
        })

        for (const filePath of annotatedFilePaths) {
            const fileIcon = this.treeView.entryForPath(filePath)
            // Avoid inline styles for easier customization.
            fileIcon.classList.add(className)
        }
    }
