...
import Renderers, {AssetRenderer} from './asset-renderers/all-renderers'
...
export default {
    ...
    registerRenderer(rendererClass) {
        const adjustedRendererClass = AssetRenderer.ensureSameApi(rendererClass)
    ...
    _initEditor(editor, editorPath) {
        ...
        const gutter = editor.addGutter({name: CodeAnnotations.GUTTER_NAME, priority: Config.gutterPriority, visible: true})
        const ranges = []
        const assetData = []
    ...
    */
    ...
    _getAnnotationRegex(grammar) {
        if (this.annotationRegexCache[grammar.name] != null) {
            return this.annotationRegexCache[grammar.name]
        }
        const patternData = CommentCharacters.format(CommentCharacters[grammar.name])
        if (patternData != null) {
            // name must begin with word character (like most common identifiers)
            this.annotationRegexCache[grammar.name] = new RegExp(
