...
export default {
    ...
    _initEditor(editor) {
        ...
        console.log('in _initEditor.......', rawAnnotations)
        ...
        const assetManager = AnnotationsDb.getInstanceForEditor(editor)
        const annotations = []
    ...
    _registerRenderers() {
        for (const name in Config.configData.renderers.properties) {
            if (Config[name] === true) {
                this.registerRenderer(Renderers[name.replace('load', '')])
            }
        }
    },
    ...
    _getRawAnnotationsForEditor(editor, useCache=true) {
        ...
        console.log('asking the db', db)
        if (db) {
            return db.getAnnotationsForEditor(editor)
        }
        return []
    },
